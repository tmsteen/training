...
class MinBalAccount(BankAccount):
    def __init__(self, init_deposit, min_bal = 250):  # Set default account min to 250
        if init_deposit < min_bal:
            raise ValueError('Initial deposit does not meet accout minimum requirement.', min_bal)
        else: 
            self.balance = 0
            BankAccount.acct_cntr += 1
