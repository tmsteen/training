...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--configfile", help="path to config file",
                        type=str, required=True)
    config = parser.parse_args()

    # Parse config file
    with open(config.configfile, 'r') as config_obj:
        yaml_obj = yaml.load(config_obj)

        if yaml_obj['thread']:
            work_q = queue.Queue()
            done_q = queue.Queue()
        else:
            work_q = multiprocessing.JoinableQueue(yaml_obj['items']+1)
            done_q = multiprocessing.JoinableQueue(yaml_obj['items']+1)
        workers = create_workers(yaml_obj['workers'], yaml_obj['thread'],
                                 yaml_obj['process'], work_q, done_q)
        work = generate_work(yaml_obj['items'])
        [w.start() for w in workers]
        start = time.time()
        [work_q.put(w) for w in work]
        returned_items = 0
        while returned_items < yaml_obj['items']:
            result = done_q.get()
